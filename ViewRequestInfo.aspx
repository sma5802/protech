...
ConnectionString="<%$ ConnectionStrings:dbConnect %>"
        ...
        SelectCommand="select (ri.fname+' '+ri.Lname) as name,ri.*,(select country from pep$tech$corp.peptech_country where id=ri.country)countryname,isnull((select state from dbo.peptech_state where id=ri.state),other)statename from dbo.peptech_RequestInfo ri where Enquirytype='Catalog Request' order by posteddate desc">
</asp:SqlDataSource>

<asp:SqlDataSource ID="srccmsdetail" runat="server"
ConnectionString="<%$ ConnectionStrings:dbConnect %>"

        SelectCommand="select (ri.fname+' '+ri.Lname) as name,ri.*,(select country from pep$tech$corp.peptech_country where id=ri.country)countryname,isnull((select state from dbo.peptech_state where id=ri.state),other)statename from dbo.peptech_RequestInfo ri where id=@id">
<SelectParameters>
